...
async def test_removed_user_source_refuses_both_regeneration_prompt_resolutions(
    ...
    with SessionLocal() as session:
        for suffix, provenance in (
            ("parts", {}),
            ("run", {"prompt_source": {"kind": "constructed"}}),
        ):
            chat = Chat(id=f"chat_remove_regen_{suffix}", title="Regeneration refusal")
            user = Message(
                id=f"msg_remove_regen_user_{suffix}",
                chat=chat,
                role="user",
                status="complete",
            )
            assistant = Message(
                id=f"msg_remove_regen_assistant_{suffix}",
                chat=chat,
                parent_id=user.id,
                role="assistant",
                status="complete",
            ...
            session.add_all(
                [
                    MessagePart(
                        id=f"part_remove_regen_user_{suffix}",
                        message_id=user.id,
                        position=0,
                        type="text",
                        text="constructed source prompt",
                        metadata_json={},
                    ),
                    MessagePart(
                        id=f"part_remove_regen_assistant_{suffix}",
                        message_id=assistant.id,
                        position=0,
                        type="text",
                        text="constructed retained reply",
                        metadata_json={},
                    ),
                    ResponseRevisionPart(
                        id=f"revpart_remove_regen_{suffix}",
                        response_revision_id=revision.id,
                        position=0,
                        type="text",
                        text="constructed retained revision",
                        metadata_json={},
                    ),
                ]
            )
    ...
    for suffix in ("parts", "run"):
        user_id = f"msg_remove_regen_user_{suffix}"
        preview = await client.get(f"/api/messages/{user_id}/removal-impact")
        removed = await client.post(
            f"/api/messages/{user_id}/remove-content",
            json=_request(
                user_id,
                preview.json()["message_revision_id"],
                f"remove-regen-{suffix}",
            ),
        )
        assert removed.status_code == 200

        regenerated = await client.post(
            f"/api/messages/msg_remove_regen_assistant_{suffix}/regenerate",
            json={"settings": {}},
        )
        assert regenerated.status_code == 409
        assert regenerated.json()["code"] == "source-content-removed"

    with SessionLocal() as session:
        for suffix in ("parts", "run"):
            assistant = session.get(Message, f"msg_remove_regen_assistant_{suffix}")
            assert assistant is not None
            assert assistant.status == "complete"
            assert [part.text for part in assistant.parts] == ["constructed retained reply"]
            revisions = list(
                session.scalars(
                    select(ResponseRevision).where(ResponseRevision.message_id == assistant.id)
                )
            )
            assert len(revisions) == 1
            assert revisions[0].status == "complete"
...
async def test_removed_run_source_refuses_every_retry_entry_without_mutation(
    ...
    with SessionLocal() as session:
        ...
        user = Message(
            id="msg_remove_retry_user",
            chat=chat,
            role="user",
            status="complete",
        )
        assistant = Message(
            id="msg_remove_retry_assistant",
            chat=chat,
            parent_id=user.id,
            role="assistant",
            status="failed",
        ...
        session.flush()
        step = WorkStep(
            id="step_remove_retry",
            plan_id=plan.id,
            ordinal=1,
            operation="text",
            status="failed",
            prompt="constructed retry prompt",
            settings_json={},
            input_bindings_json=[],
        ...
        run = Run(
            ...
            status="failed",
            standalone_prompt="constructed retry prompt",
            provenance_json={},
            settings_json={},
        )
        session.add(run)
        ...
        session.add_all(
            [
                MessagePart(
                    id="part_remove_retry_user",
                    message_id=user.id,
                    position=0,
                    type="text",
                    text="constructed source payload",
                    metadata_json={},
                ),
                MessagePart(
                    id="part_remove_retry_assistant",
                    message_id=assistant.id,
                    position=0,
                    type="error",
                    text="constructed prior failure",
                    metadata_json={},
                ),
                Job(
                    id="job_remove_retry",
    ...
    preview = await client.get("/api/messages/msg_remove_retry_user/removal-impact")
    removed = await client.post(
        "/api/messages/msg_remove_retry_user/remove-content",
        json=_request(
            "msg_remove_retry_user",
            preview.json()["message_revision_id"],
            "remove-retry-source",
        ),
    )
    ...
    for endpoint in (
        "/api/jobs/job_remove_retry/retry",
        "/api/work-steps/step_remove_retry/retry",
        "/api/work-plans/plan_remove_retry/retry",
        "/api/jobs/job_remove_retry_verify/retry",
    ):
        response = await client.post(endpoint)
        assert response.status_code == 409
        assert response.json()["code"] == "source-content-removed"

    with SessionLocal() as session:
        job = session.get(Job, "job_remove_retry")
        verification = session.get(Job, "job_remove_retry_verify")
        run = session.get(Run, "run_remove_retry")
        assistant = session.get(Message, "msg_remove_retry_assistant")
        ...
        assert [(part.type, part.text) for part in assistant.parts] == [
            ("error", "constructed prior failure")
        ]
