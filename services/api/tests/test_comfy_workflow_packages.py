...
def test_a_package_the_runtime_did_not_load_is_not_resolved() -> None:
    """Reviewed evidence says a package owns these nodes. The runtime says it
    never loaded them. The runtime wins, because it is the only one of the two
    that describes what will happen when the workflow runs.

    This mattered on a real machine: recording a package's reviewed inventory
    made it read as installed, which removed the repair action from the review
    screen, while the package still could not import for a missing Python
    dependency. The badge said installed, the remedy was hidden, and the import
    refused - all at once.
    """
    ...
    analysis = analyze_comfyui_workflow_package(
        value,
        # Evidence claims the node; the runtime loaded only the core one.
        available_node_types={"KSampler", "CustomNode"},
        installed_package_versions={"example-pack": {"a" * 40}},
        runtime_node_types={"KSampler"},
    )
...
def test_evidence_still_resolves_a_package_when_there_is_no_runtime_to_ask() -> None:
    """Without a runtime the evidence is all there is, and refusing everything
    would make the review screen useless while the worker is stopped."""
