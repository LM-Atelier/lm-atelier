...
def test_tags_normalize_once_and_assign_explicitly(
    ...
    assert tag.normalized_name == "portrait-study"
...
def test_raw_sql_cannot_forge_identities_versions_or_dangling_references(
    ...
    statements: tuple[tuple[str, dict[str, object]], ...] = (
        ...
        (
            "UPDATE media_tags SET normalized_name='changed',version=version+1 WHERE id=:id",
            {"id": tag.id},
        ),
        (
        ...
        (
            ...
            "(id,normalized_name,label,color,version,created_at,updated_at) "
...
def test_schema_rejects_duplicate_order_and_normalized_tag_name(
    organization_session: tuple[ArtifactStore, Session],
) -> None:
    store, session = organization_session
