...
@pytest.mark.parametrize("kind", ["system", "accelerator"])
def test_a_declared_minimum_keeps_its_declared_basis(kind: str) -> None:
    result = recommend_hardware_fit(
        _capacity(),
        FitRequirements(
            minimum_system_memory_bytes=8 * _GIB if kind == "system" else None,
            minimum_accelerator_memory_bytes=8 * _GIB if kind == "accelerator" else None,
        ),
    )

    assert result.status == "likely"
    assert result.basis == "declared"
    assert result.resources[0].basis == "declared"
    assert not any(reason.code.endswith("_estimated") for reason in result.reasons)


@pytest.mark.parametrize("kind", ["system", "accelerator"])
def test_an_estimate_above_total_capacity_does_not_claim_the_model_fits(kind: str) -> None:
    result = recommend_hardware_fit(
        _capacity(),
        FitRequirements(
            estimated_system_memory_bytes=64 * _GIB if kind == "system" else None,
            estimated_accelerator_memory_bytes=32 * _GIB if kind == "accelerator" else None,
        ),
    )

    assert result.status == "tight"
    assert result.resources[0].immediate_pressure
    busy = next(reason for reason in result.reasons if reason.code == f"{kind}_memory_busy")
    assert "fits total" not in busy.message
    assert "currently free" in busy.message
