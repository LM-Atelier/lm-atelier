...
from local_lm.prompt_binding import binds_prompt, frozen_prompt, ignores_the_description
...
def test_a_node_holding_words_of_its_own_has_a_frozen_prompt() -> None:
    assert frozen_prompt(_graph("a post-rain dusk metropolis")) is True
    assert frozen_prompt({"n": {"inputs": {"prompt": "an action movie trailer"}}}) is True
...
def test_a_graph_with_no_prompt_input_at_all_freezes_nothing() -> None:
    ...
    """
    assert frozen_prompt({"1": {"class_type": "LoraLoader", "inputs": {"lora_name": "x"}}}) is False
    assert (
        frozen_prompt(
    ...
    assert frozen_prompt({"node": {"class_type": "TestOutput"}}) is False
    assert frozen_prompt({}) is False


def test_an_empty_or_placeholder_value_is_not_words_of_its_own() -> None:
    assert frozen_prompt(_graph("")) is False
    assert frozen_prompt(_graph("   ")) is False
    assert frozen_prompt(_graph("${prompt}")) is False
    # A binding to some other input is still a binding, not an author's words.
    assert frozen_prompt(_graph("${style}")) is False
...
def test_a_graph_that_offers_no_prompt_is_left_alone(operation: str) -> None:
    ...
    assert ignores_the_description("comfyui", operation, fragment) is False
...
def test_a_bound_positive_prompt_makes_a_fixed_negative_one_ordinary() -> None:
    ...
    assert frozen_prompt(graph) is True
    assert ignores_the_description("comfyui", "text_to_image", graph) is False
