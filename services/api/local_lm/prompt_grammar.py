...
_IDENTIFIER_PATTERN = r"@?[A-Za-z0-9][A-Za-z0-9_.-]{0,62}"
_IDENTIFIER = re.compile(_IDENTIFIER_PATTERN)
# The template is scanned with the same identifier grammar as the declarations,
# built from the same pattern. Two scanners that disagree is not a style
# problem: a hyphen is legal inside an identifier, so a separate punctuation
# alphabet containing "-" tore a legal trigger in half and then reported the
# halves as prose.
_TEMPLATE_TOKEN = re.compile(rf"<([^<>]{{1,64}})>|({_IDENTIFIER_PATTERN})")
# Everything between tokens. "-" is absent on purpose; it belongs to identifiers.
_TEMPLATE_PUNCTUATION = frozenset(" \t,;:.()[]{}/|<>\"'")
# The slot every grammar has, and the only one this module names itself. It is
# where the described scene goes, so a template without it has nowhere to put
# the answer.
...
def _parse_template(template: object, trigger: str, slots: tuple[GrammarSlot, ...]) -> str:
    ...
    words: list[str] = []
    position = 0
    for match in _TEMPLATE_TOKEN.finditer(cleaned):
        gap = set(cleaned[position : match.start()]) - _TEMPLATE_PUNCTUATION
        if gap:
            raise PromptGrammarError(f"template contains {''.join(sorted(gap))[:16]!r}")
        name, word = match.group(1), match.group(2)
        if name is not None:
            if not _IDENTIFIER.fullmatch(name):
                raise PromptGrammarError(f"placeholder {name[:32]!r} is not a plain identifier")
            placeholders.append(name)
        else:
            words.append(word)
        position = match.end()
    trailing = set(cleaned[position:]) - _TEMPLATE_PUNCTUATION
    if trailing:
        raise PromptGrammarError(f"template contains {''.join(sorted(trailing))[:16]!r}")

    if len(placeholders) != len(set(placeholders)):
        raise PromptGrammarError("template repeats a placeholder")
    if placeholders.count(DESCRIPTION_SLOT) != 1:
        raise PromptGrammarError(
            f"template must place <{DESCRIPTION_SLOT}> exactly once; it is where the "
            "described scene goes"
        )

    # Every declared slot must appear. The weaker rule - only required ones -
    # let a grammar declare a slot the template had nowhere to put, and then a
    # rewriter would advertise a value it could not position.
    declared = {slot.name for slot in slots}
    positioned = set(placeholders) - {DESCRIPTION_SLOT}
    unknown = sorted(positioned - declared)
    if unknown:
        raise PromptGrammarError(f"template uses undeclared placeholder(s): {', '.join(unknown)}")
    missing = sorted(declared - positioned)
    if missing:
        raise PromptGrammarError(f"template gives no place to: {', '.join(missing)}")

    # Whatever is not a placeholder must be the trigger. A word here is prose,
    # and prose in this position writes part of a rewriter's instruction.
...
def normalize_grammar(
    ...
    *,
    ...
    for entry in raw_slots:
        ...
        if name == DESCRIPTION_SLOT:
            raise PromptGrammarError(
                f"{DESCRIPTION_SLOT!r} is reserved for the described scene and cannot "
