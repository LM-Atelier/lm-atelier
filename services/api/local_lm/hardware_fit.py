...
def _assess_memory(
    *,
    ...
    if measured_peak_bytes is not None and evidence_claim is not None:
    ...
    elif minimum_bytes is not None and estimated_bytes is None:
        basis = "declared"
        ...
        reasons.append(
            FitReason(
                ...
                f"Hardware meets the declared minimum {label} capacity.",
            )
        )
    else:
        basis = "calculated"
    ...
    if immediate_pressure:
        reasons.append(
            FitReason(
                f"{kind}_memory_busy",
                "warning",
                f"The required {label} exceeds currently free capacity.",
            )
        )
    return _MemoryAssessment(
        status,
