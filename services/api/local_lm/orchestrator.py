...
class ConversationOrchestrator:
    ...
    async def _execute_media(self, job_id: str, run_id: str, claim: JobClaim) -> str | None:
        ...
        with self.session_factory() as session:
            ...
            if not run:
                return None
            validated_revision_id = run.workflow_revision_id
            selected_revision = (
        ...
        with self.session_factory() as session:
            ...
            if not run:
                return None
            if run.workflow_revision_id != validated_revision_id:
        ...
        try:
            ...
            # The producer is typed as an iterator by the adapter contract, and a
            # close that raises or hangs must not replace the refusal that
    ...
    def _bound_retry_announcement(
        self, session: Session, accepted: TurnAccepted
    ) -> tuple[dict[str, Any], list[tuple[str, str]]] | None:
        """What announcing an already durable retry needs, read in one place.

        Returns the plan event and the queued, unclaimed jobs to start, or
        None when the retry's run or plan is gone. Reading is separated from
        announcing so a caller that owns its session can let it go before the
        announcement's await.
        """
        ...
        for job in jobs:
            if job.status == JobStatus.QUEUED.value and job.claim_owner is None and job.run_id:
                queued.append((job.id, job.run_id))
        return event, queued

    async def _announce_bound_retry(
        self, event: dict[str, Any], queued: list[tuple[str, str]]
    ) -> None:
        """Announce a durable retry and start its queued jobs. Both are
        idempotent for a queued job, so a repeat costs nothing."""

        await self.events.publish("work_plan.created", event["plan_id"], event)
        for job_id, run_id in queued:
            self.start(job_id, run_id)
    ...
    async def _resume_bound_retry(self, session: Session, accepted: TurnAccepted) -> TurnAccepted:
        ...
        announcement = self._bound_retry_announcement(session, accepted)
    ...
    async def _converge_on_bound_retry(
        ...
        with self.session_factory() as session:
            ...
            try:
                ...
                # Reading what to announce is part of the best-effort
                # recovery, not of the binding: a transient read failure
                # here must return the retry the source already bound, or
                # the caller records the verification unavailable and
