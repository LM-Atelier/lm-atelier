...
BEGIN
  ...
  SELECT CASE WHEN NEW.width IS NULL OR NEW.height IS NULL
                        OR typeof(NEW.width) != 'integer'
                        OR typeof(NEW.height) != 'integer'
                        OR typeof(NEW.review_version) != 'integer'
...
BEGIN
  SELECT CASE WHEN EXISTS (
    SELECT 1 FROM reference_asset_review_events
    WHERE id = NEW.id
       OR (reference_asset_id = NEW.reference_asset_id
           AND result_version = NEW.result_version)
  ) THEN RAISE(ABORT, 'reference asset review event already exists') END;
  ...
  SELECT CASE WHEN NEW.reviewer_kind != 'local-human'
                        ...
                        OR typeof(NEW.width) != 'integer'
                        OR typeof(NEW.height) != 'integer'
                        OR typeof(NEW.expected_version) != 'integer'
                        OR typeof(NEW.result_version) != 'integer'
                        OR NEW.width < 1 OR NEW.height < 1
